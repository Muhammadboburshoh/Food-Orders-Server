...
create function new_order() returns trigger language plpgsql as $$
  ...
  begin
    if (select status::boolean from orders where table_id = NEW.table_id and status = 0) then
      insert into order_item (order_id, product_id, count) values
      (select order_id from orders where table_id = NEW.table_id and status = 0, NEW.product_id, NEW.count);
    else
      return NEW;
    end if;
  end;
